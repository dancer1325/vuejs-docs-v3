...
* guide content
  * 👀-- depends on -- Options API OR Composition AP👀
...
* `data`
  * == option /
    * declare component's reactive state 
    * 's value
...
* You should avoid using names for top-level `data` properties that start with either of these characters.
...
**Unlike in Vue 2, the original `newObject` is left intact and will not be made reactive: make sure to always access reactive state as a property of `this`.**
